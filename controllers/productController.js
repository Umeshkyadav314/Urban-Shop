...
import fs from 'fs'
import categorModel from '../models/categoryModel.js'
...
export const deleteProductController = async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      message: 'Error while products deleted',
      error
    })
  }
}
...
export const updateProductController = async (req, res) => {
  try {
    ...
    res.status(500).send({
      success: false,
      error,
      message: 'Error while updated products'
    })
  }
};
...
export const productCategoryController = async (req, res) => {
  try {
    const category = await categorModel.findOne({ slug: req.params.slug })
